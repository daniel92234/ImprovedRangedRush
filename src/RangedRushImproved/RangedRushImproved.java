...
//        System.out.println("LightRushAI for player " + player + " (cycle " + gs.getTime() + ")");
    ...
    public void baseBehavior(Unit u, Player p, PhysicalGameState pgs) {
        ...
        if (nworkers < 4 && p.getResources() >= workerType.cost) { // Make three workers
            train(u, workerType);
        }
    }
...
//            System.out.println("LightRushAI.meleeUnitBehavior: " + u + " attacks " + closestEnemy);
    ...
    public void workersBehavior(List<Unit> workers, Player p, GameState gs) {
        ...
        if (nbarracks == 0 && !freeWorkers.isEmpty()) {
            // build (a) barrack(s):
            if (p.getResources() >= barracksType.cost + resourcesUsed && nbarracks < 3) { // Limit to two barracks
                Unit u = freeWorkers.remove(0);
        ...
        for (Unit u : freeWorkers) {
            ...
            if (u.getResources() > 0) {
                ...
                if (closestResource!=null && closestBase!=null) {
                    ...
                    if (aa instanceof Harvest) {
                        Harvest h_aa = (Harvest)aa;
                        if (h_aa.getTarget() != closestResource || h_aa.getBase()!=closestBase) harvest(u, closestResource, closestBase); // Member values target and base are private. Using accessors instead...
                    } else {
